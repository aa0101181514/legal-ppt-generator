...
3. Project 名稱填：`法律書狀 PPT 助手`（或你喜歡的名稱）

### 步驟 2：從 GitHub 匯入本 repo

1. 在 Project 頁面右側找到 **Files** 區塊
2. 點 **+** → 選 **GitHub**
3. 第一次使用需要授權 Claude 存取你的 GitHub（只讀權限）
4. Repo 欄位貼上：
   ```
   aa0101181514/legal-ppt-generator
   ```
   （或直接貼 `https://github.com/aa0101181514/legal-ppt-generator`）
5. 勾選要匯入的項目：
   - ✅ `PROJECT_INSTRUCTIONS.md`（**必勾 — 系統指令**）
   - ✅ `knowledge/`（整個資料夾勾起來）
   - ⬜ `README.md`（可不勾，對 Claude 無用）
6. 點右下角 **Add to project**

完成。**不需要設定 Instructions 欄位**（系統指令已經在匯入的檔案裡）。
